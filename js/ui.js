...
const REGIONS_DB = {
    "인천": ["인천"],
    "남양주/구리": ["남양주", "구리", "MNYJ"],
    "시흥/안산": ["시흥", "안산", "부천", "군포", "안양"],
    "용인/광주": ["용인", "이천", "경광주", "곤지암", "마장"],
    "안성/여주": ["안성", "여주", "M안성"],
    "천안/세종": ["천안", "아산", "세종", "성환", "성거", "XRC", "XHM"],
    "고양/김포": ["고양", "MGMP"]
};

function getRegionName(name) {
    for (const [r, keywords] of Object.entries(REGIONS_DB)) {
...
function getRoles() {
    ...
    Object.keys(roles).forEach(c => {
        ...
        else result[result[c] = 'role-both'];
    });
    return result;
}
...
export function initQuickButtons() {
    const tabBox = document.getElementById('quick-region-tabs');
    const chipBox = document.getElementById('quick-center-chips');
    if(!tabBox) return;

    const roles = getRoles();
    const groups = { "기타": [] };
    Object.keys(REGIONS_DB).forEach(r => groups[r] = []);
    MEM_CENTERS.forEach(c => groups[getRegionName(c)].push(c));
    ...
    Object.keys(groups).forEach(region => {
        ...
        btn.onclick = (e) => {
            e.preventDefault();
            tabBox.querySelectorAll('.region-btn').forEach(b => b.classList.remove('active'));
            ...
            groups[region].sort().forEach(c => {
                ...
                chip.textContent = c;
                chip.onclick = () => {
                    const fromInp = document.getElementById('from-center');
                    const toInp = document.getElementById('to-center');
                    if (!fromInp.value) fromInp.value = c;
                    else toInp.value = c;
                    fromInp.dispatchEvent(new Event('input'));
                };
...
// 설정창 지역관리 아코디언 출력 (기존 displayCenterList를 덮어씁니다)
export function displayCenterList(filter='') {
    ...
    Object.keys(groups).sort().forEach(region => {
        ...
        header.onclick = () => {
            const act = content.classList.toggle('active');
            header.querySelector('span:last-child').textContent = act ? '▲' : '▼';
        };
        groups[region].forEach(c => {
            const div = document.createElement('div');
            div.className = 'center-item';
            div.innerHTML = `<span>${c}</span><button onclick="window.extDeleteCenter('${c}')">삭제</button>`;
            content.appendChild(div);
        });
        groupDiv.appendChild(header); groupDiv.appendChild(content); container.appendChild(groupDiv);
    });
}
window.extDeleteCenter = (name) => {
    if(confirm('삭제?')) {
        const idx = MEM_CENTERS.indexOf(name);
        if(idx > -1) MEM_CENTERS.splice(idx, 1);
        saveData(); displayCenterList();
    }
};