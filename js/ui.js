...
export function displayCenterList(filter='') {
    ...
    Object.keys(groups).sort().forEach(region => {
        ...
        groups[region].forEach(c => {
            ...
            div.innerHTML = `<div class="info"><span>${c}</span><div class="action-buttons"><button class="del-btn" style="background:#dc3545; color:white; border:none; padding:3px 8px; border-radius:4px;">삭제</button></div></div>`;
            div.querySelector('.del-btn').onclick = () => {
                if(confirm('삭제?')) { MEM_CENTERS.splice(MEM_CENTERS.indexOf(c), 1); saveData(); displayCenterList(filter); }
            };
...
export function toggleUI() {
    ...
    if (type === '화물운송') {
        els.transportDetails.classList.remove('hidden'); els.costInfoFieldset.classList.remove('hidden'); els.incomeWrapper.classList.remove('hidden'); els.costWrapper.classList.add('hidden');
...
export function resetForm() {
    els.recordForm.reset(); els.editModeIndicator.classList.add('hidden');
